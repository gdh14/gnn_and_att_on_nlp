...
def learning_rate_decay(optimizer):
    for param_group in optimizer.param_groups:
        param_group['lr'] = max(param_group['lr'] * 0.9, 0.000001)
    return optimizer
...
if args["model"] == 'lstm':
    ...
    model = DDCNN(embed_dim, nhid, len(w2i), nclass, kernel_size, rez_block=True)
...
# if args["model"] == 'lstm':
#     from models.classifiers import LSTM_clf
#     model2 = LSTM_clf(embed_dim, nhid, vocab_size, nclass, nlayers)
    
# elif args["model"] == 'transformer':
#     from models.classifiers import TransformerModel
#     model2 = TransformerModel(nclass, embed_dim, nhead, nhid, nlayers, len(w2i), dropout)
    
# elif args["model"] == "ddcnn":
#     from models.classifiers import DDCNN
#     model2 = DDCNN(embed_dim, nhid, len(w2i), nclass, kernel_size, rez_block=True)
